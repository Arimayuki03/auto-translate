...
describe("splitBatch（哨兵分隔）", () => {
  ...
  it("哨兵模式：以时间/小数开头的译文不再被误剥（2026-09-26 修复）", () => {
    // 旧实现「对每段无条件 stripIndex」会把 "10:30 出发" 剥成 "30 出发"、"2.99 美元" 剥成
    // "99 美元"，损坏值还会写进缓存永久复用。新实现只在全段 1..N 连续递增时才剥。
    expect(splitBatch(`10:30 出发\n${SEP}\n第二段`, 2)).toEqual(["10:30 出发", "第二段"]);
    expect(splitBatch(`2.99 美元\n${SEP}\n共两段`, 2)).toEqual(["2.99 美元", "共两段"]);
  });
...
describe("splitBatch（行匹配兜底 / 兼容旧行为）", () => {
  ...
  it("部分行带编号时不再剥离（新语义：部分命中时编号更可能是译文本身）", () => {
    // 旧语义「部分命中就剥」会误伤以时间/小数/价格开头的译文（"10:30 出发" → "30 出发"）。
    // 新语义：只有全段都带编号且恰为 1..N 连续递增才剥；部分命中原样保留。
    expect(splitBatch("1. a\nb", 2)).toEqual(["1. a", "b"]);
    expect(splitBatch("a\n2. b", 2)).toEqual(["a", "2. b"]);
  });
...
describe("splitBatch（边界场景：换行 / 空段 / 编号 / Markdown / 代码）", () => {
  ...
  it("哨兵模式：编号非 1..N 连续递增时原样保留（新语义）", () => {
    const out = `第 1 步：准备\n${SEP}\n2. 这是合法的第二段开头`;
    // 第一段中段数字保留；第二段段首 "2." 因不满足「全段 1..N 连续递增」而原样保留——
    // "2." 更可能是译文本身（如列表项、时间）而非模型自加序号，剥了就是错译。
    // 旧语义会把它剥掉（"这是合法的第二段开头"），已被 2026-09-26 splitBatch 收紧取代。
    expect(splitBatch(out, 2)).toEqual(["第 1 步：准备", "2. 这是合法的第二段开头"]);
  });
